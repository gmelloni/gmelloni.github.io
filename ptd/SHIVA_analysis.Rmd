...
The trial was aiming at compare conventional therapy with 
...
that the R package ```PrecisionTrialDesigner``` 
(here-hence referred as **PTD**) can be applied for predicting 
...
If molecular abnormality is identified, patients are randomized between 
...
#import SHIVA Design
# --------------------------------------------------------------------------------
SHIVA_design <- readxl::read_excel("Supplementary_file/SHIVA_design_v6.1.xlsx"
                                    , sheet = 1
...
Import in the analysis the raw data. Add a column with the TCGA tumor ID conversion ```Supplementary_file/SHIVA_conversiontable.xlsx```.
...
of the remaining now, you can check how many are ```eligible for randomization``` and therefore calculate the expected detection power. 
...
The following table shows the patients constituting the SHIVA population for this analysis.
...
The population recruited in the trial is composed by different types of tumor types (after conversion to TCGA tumor ids). The following barplot shows the number of patient for each tumor type.
...
From the 741 patients from the study, ```r nrow(merged_df)``` are eligible for randomization. After removing patients where next-generation squecing was not performed we were left with 
 ```r merged_df %>% dplyr::filter(!NGS == "Not done") %>% nrow()```. We then selected only those patients without tumor types compatible with TCGA tumor ids: ```r merged_df %>% dplyr::filter(!NGS == "Not done") %>% dplyr::filter(!TCGA_tumor_ID == "") %>% nrow()``` . 
Finally, we removed patients with TCGA tumor types  acyc, sclc, es, because have no CNA data in TCGA population. Final number of patients  ```r nrow(df_full_profile)```.
...
For each gene, in each patient, it was reported a CNA status. For a small percentage of cases, the value reported was ambiguous (e.g. "amplification, normal, delete_het").
...
```{r SHIVA repo, warning=FALSE, message=FALSE}
# DESIGN
...
saveRDS(SHIVA_population, file = "Supplementary_file/SHIVA_population.rds")
```
...
#save a backup copy to speed up future analysis
saveRDS(TCGA_population, file = "Supplementary_file/TCGA_population.rds")
```

```{r, echo=FALSE}
TCGA_population <- readRDS("Supplementary_file/TCGA_population.rds")
```
...
cosmic <- readr::read_csv("Supplementary_file/external_resources/cosmic_pathogenic_SHIVApanel")
...
PATH_2CNA_FILES <- "Supplementary_file/cna_firehose_rds/"
...
Preview table with the contribution to the overall detection power by only mutation alterations.
...
Preview table with the contribution to the overall detection power by only CNA alterations.
...
Preview plot with the contribution to the overall detection power by only mutation and CNA alterations separately
...
datatable_withbuttons(cov, caption="Datatable with detection power from Mutations for the SHIVA population")
```
...
datatable_withbuttons(cov, caption="Datatable with detection power from CNA for the SHIVA population")

```


#### Merge the Detection power from the 2 population in one plot

```{r}
# plot two pics separated
#fig4a <- gridExtra::grid.arrange(TCGA_detection_image, SHIVA_detection_image)

# plot everything in one graph
# ------------------------------------------------------------------------------
dtpow$population <- "TCGA"

dtpow_shiva %<>%
  dplyr::rename(detectionPower=shiva_freq) %>%
  dplyr::mutate(population = "SHIVA")

# plot the dataframe merged
fig4b <- dplyr::bind_rows(dtpow, dtpow_shiva) %>%
  ggplot(aes(x=n_alterations, y=detectionPower, fill=population)) + 
  geom_bar(stat="identity" , position=position_dodge()) + 
  scale_fill_manual(values=MYPALETTE(10)[c(10,2)]) +
  ...
  ggtitle("Overall Detection power in SHIVA and TCGA Population") +
  geom_text(aes(label=sapply(detectionPower*100
                             , function(x) 
                                  paste(c(round(x, digits=2), "%"), collapse=" ")
                      )
                , x=n_alterations
                , y=detectionPower)
...
# Make the plot
fig4c_ex <- gridExtra::grid.arrange(CNA_plot_amplification, CNA_plot_deletion, ncol=1, heights = c(20,5))
...
In this part of the analysis we are comparing the patient allocation to the two molecular pathway treatments. 


Show the maximum  patient allocation to the molecular pathway treatment. This is theoretical as, it presumes that one patient can be allocated to more than one treatment. There is in fact a small percentage of patients with a set of alteration that makes them eligible for both molecular treatments.  
...
In case of a patient eligible for both molecular pathways treatment, the SHIVA medical board would decide in with cohort to allocate the patient. To allow a better comparison, the following plot separates the patients eligible to both treatments. The y-axes shows the tot number of patients that can be allocated in a molecular pathway treatment. 
...
fig4C <- merge_df %>%
  ...
  scale_fill_manual(values=MYPALETTE(10)[c(10,2)]) + 
  geom_errorbar(aes(min=lowerCI, max=upperCI), size=0.2) + 
...
fig4C <- fig4C + geom_text(aes(x=(row_counter)
                  , y=upperCI+.05
...
```{r, warning=FALSE}
# Shiva full design
shivaSurv <- survPowerSampleSize(SHIVA_population
                    , alterationType = c("mutations", "copynumber")
                    , HR = 0.625
...
```{r, warning=FALSE}
tcgaSurv <- survPowerSampleSize(TCGA_population
                    , alterationType = c("mutations", "copynumber")
                    , HR = 0.625
                    , power = seq(0.6 , 0.9 , 0.1)
                    , alpha = 0.05
                    , case.fraction = 0.5
                    , noPlot = TRUE)
knitr::kable(tcgaSurv)
```

##### SHIVA vs TCGA

```{r}
toBePlot <- rbind( cbind(shivaSurv , Study = "SHIVA") , cbind(tcgaSurv , Study = "TCGA"))
shivaTcgaFull <- ggplot(toBePlot , aes(y=Power, x=ScreeningSampleSize, colour=Study , group=Study)) +
...
#Export as a svg
#img_file = "Figures/tcgaVSshivaFullDesign_surv.svg"
#ggsave(filename=img_file, plot=shivaTcgaFull, device = "svg")
```
...
fullDesignComparison <- data.frame(ScreeningSampleSize_TCGA = tcgaFullDesign$ScreeningSampleSize
                                ...
                                , Power = seq(0.6 , 0.9 , 0.1)
                                , HR = 0.625)
datatable_withbuttons(fullDesignComparison , rownames = FALSE)
```
...
since it relies on the assumption of independent studies. 
From this point on we will report the same value used in the design of the SHIVA trial:
...
shivaByDrug <- survPowerSampleSize(SHIVA_population
   ...
   , var = "group" 
   , noPlot=TRUE)
Fig4D <- survPowerSampleSize(SHIVA_population
   , alterationType = c("mutations", "copynumber")
   , HR = 0.625
   ...
   , fu=1.883
   #, tumor.freqs = tumor_freq
   , var = "group") + ggtitle("SHIVA: design by drug category (not optimized)")
Fig4D
datatable_withbuttons(shivaByDrug , rownames=FALSE)
```
...
tcgaByDrug <- survPowerSampleSize(TCGA_population
   ...
   , var = "group" 
   , noPlot=TRUE)
tcgaByDrugPlot <- survPowerSampleSize(TCGA_population
   , alterationType = c("mutations", "copynumber")
   , HR = 0.625
   , power = seq(0.7 , 0.9 , 0.1)
   , alpha = 0.05
   ...
   , fu=1.883
   , tumor.freqs = tumor_freq
   , var = "group") + ggtitle("TCGA: design by drug category (not optimized)")
tcgaByDrugPlot
datatable_withbuttons(tcgaByDrug , rownames=FALSE)
```

```{r , echo=FALSE, message=FALSE}
#Export as a svg
#img_file = "Figures/tcgaByDrugNotOptim_surv.svg"
#ggsave(filename=img_file, plot=tcgaByDrugPlot, device = "svg")
```
...
*Confidence Intervals* from the bootstrap simulation are indicated in parenthesis. 
...
and discarding `r summaryMat[3,1]` (the former are the patients 
not eligible to any molecular drug treatments). 
The non eligible patients of `r colnames(summaryMat)[1]` might be eligible 
for other drug categories, so we calculate whether within these patients, 
there is an enough number of eligible patients to reach the required power. 
In case the number is sufficient, the algorithm indicates 
how many more patients are necessary to recruit. 
Based on our calculations no further patients are necessary.  
Finally, the last column indicates the total number 
of patients screened, eligible, and non eligible. 
...
The number of recruited patients falls into the CI of the TCGA simulation and result is therefore compatible.
...
* 496 Patients with a complete profile entered in the study
  * 203 NOT eligible
  * 293 eligible
* 46 (molecular target) + 43 (physician's choice) = 89 in PI3K/AKT/mTOR pathway arm
* 13 (molecular target) + 11 (physician's choice) = 24 in RAF/MEK pathway arm
...
size of the original SHIVA study is the lack of the entire Hormone arm that decreases the detection power in this 
...
```{r , echo=FALSE , message=FALSE}
#svg(filename="Figures/postHocPowerShiva.svg", 
#    width=7, 
#    height=7, 
#    pointsize=12)
plot(mypowers , mySampSizeDiff , pch=20 , col="black")
...
It's pretty clear that we have a minimum at `r mypowers[which.min(mySampSizeDiff)]` 
when the sample size calculated is 
equal to `r mySampSizeDiff[which.min(mySampSizeDiff)]+496`. 
This function is clearly convex in the interval (0-1) 
of the possible power levels, so the post-hoc power resides 
between 0.35 and 0.45 (according to our plot points).

For a more robust optimization, we are going to use 
a linear optimizer. To facilitate the process, we 
will initialize a value of 0.4 and lower and upper 
bounds of 0.35 and 0.45 (as calculated by our graphical method)

Since we are dealing with a one-dimensional problem with 
box constrains, we are going to use the Brent method.
...
So with an approximated screened sample size of 
`r plugInSampleSize["Screened","Total"]` we obtain 
`r plugInSampleSize["Eligible" , "Total"]` eligible patients 
divided in `r plugInSampleSize["Eligible","RAF/MEK"]` 
for RAF/MEK pathway arm and `r plugInSampleSize["Eligible","PIK3/AKT/mTOR"]` 
for PIK3/AKT/mTOR pathway arm.
...
# Make the plot
all_fig4 <- gridExtra::grid.arrange(grid::textGrob("A"), fig4b, fig4C, Fig4D
                                    , ncol=2
                                    , heights = unit(c(4,4), c("in", "in"))
                                    )

#Export as a svg
#img_file = "Figures/fig4.svg"
#ggsave(filename=img_file, plot=all_fig4, device = "svg", width=16, height = 16)
```
